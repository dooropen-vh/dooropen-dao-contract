...
/// @title the platform token. DOC token

contract DOC is ERC20, AccessiblePlusCommon, IDOC {
    ...
    /// @dev constructor of DOC, ERC20 Token
    constructor(
        string memory name_,
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "DOC: permit EXPIRED");

        bytes32 digest =
            hashPermit(owner, spender, value, deadline, nonces[owner]++);

        require(owner != spender, "DOC: approval to current owner");

        // if (Address.isContract(owner)) {
        //     require(IERC1271(owner).isValidSignature(digest, abi.encodePacked(r, s, v)) == 0x1626ba7e, 'Unauthorized');
        // } else {
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress != address(0), "DOC: Invalid signature");
        console.log("address: %s", recoveredAddress);
        require(recoveredAddress == owner, "DOC: Unauthorized");
        // }
